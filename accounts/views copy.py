...
def get_sub_category(request):
    ...
    sub_cats_list = db.configuration.distinct('name',{"classification":"sub_category","category":{"$in":category_list}})
...
def manage_product(request):
    ...
    return render(request,'manage_product.html',{'brands':brands_list,'categories':categories_list,'sub_cats':sub_cats_list})
    # return render(request,'manage_product_new.html',{'brands':brands_list,'categories':categories_list,'sub_cats':sub_cats_list})

    
@login_required(login_url='/')
def download_excel(request):

    df =  pd.DataFrame(list(excel_queryset))
        ...
        if col in price_columns:
            for ind,val in enumerate(df[col]):
                if val not in ('',0,None):
                    try:
                        df[col][ind] = "£{:,.2f}".format(cl_prc(val))
                    except:
                        pass
    for ind,val in enumerate(df['discount']):
        if val not in ('',0,None):
            try:
                df['discount'][ind] = str(val)+"%"
            except:
                pass
...
def get_products(request):
    ...
    # the below line should be called dynamically once website column in db is cleaned. 
    # website_list_1 = ['cudoni','lampoo','realreal','vestiaire']
    website_list_1 = []
    # website_list_1 =[]
    if request.POST['website'] !='':
        website_list_1 = str(request.POST['website']).strip().split('|')
        website_list = [re.compile(f"^{x.strip()}",flags=re.IGNORECASE) for x in website_list_1]
    ...
    if request.POST['post_status'] !='':
        ...
        post_status_list = [re.compile(f"^{x.strip()}",flags=re.IGNORECASE) for x in post_status_list]

        quer_dict['status'] = { '$in': post_status_list}
        # print(post_status_list)
   
    sub_cat_list=''
    if request.POST['sub_cat'] !='':
        sub_cat_list = str(request.POST['sub_cat']).strip().split('|')
        sub_cat_list = [re.compile(f"^{x.strip()}",flags=re.IGNORECASE) for x in sub_cat_list]
    ...
    if request.POST['search[value]'] !='':
        ...
        quer_dict["$or"] =  [
                                ...
                                { "condition2": re.compile(f"{search_val}",flags=re.IGNORECASE)},
    ...
    global excel_queryset
    excel_queryset =  collection.find(quer_dict).sort(col_name[col_index],col_dir)

    #products = products.collation({"locale": "en"}).sort(col_name[col_index],col_dir)
    ...
    if len(website_list_1)>0:
        for website in website_list_1:
            ...
            for i in site_specific_prices:
                ...
                site_data[(website)]['avg_price'] = "£{:,.2f}".format(cl_prc(i['max_price'])+cl_prc(i['max_price'])/2)
